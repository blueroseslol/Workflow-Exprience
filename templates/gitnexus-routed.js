...
const CHECKPOINT_KEY = workflowCheckpointKey(`nochg:${fnv1aHex(TASK)}::${MILESTONE}`, args)
...
let planDigest = [
  ...
  `切片：\n${effectivePlan.slices.map(s => `- ${s.id ?? '?'} ${s.title}（${s.files.join(', ')}）— ${s.rationale} execution=${JSON.stringify(s.execution??null)}`).join('\n')}`,
...
const implementationSchedule = buildImplementationSchedule(effectivePlan, args)
log(`Implement mode=${implementationSchedule.mode}: ${implementationSchedule.reason}`)
if (implementationSchedule.mode === 'blocked') return withPlanState({status:'blocked',at:'ImplementationSchedule',reason:implementationSchedule.reason})
if (implementationSchedule.mode === 'parallel') {
  const result = await runParallelImplementation(effectivePlan, { args, model: implementationModel, schema: IMPLEMENT_SCHEMA, history: advisorHistory,
    context: `task=${TASK}; repo=${REPO}; worktree=${WORKTREE}; GitNexus repo=${GNX}`, decided: decidedDigest }, implementationSchedule)
  advisorCalls = advisorHistory.length
  if (result.status !== 'completed') return withPlanState({...result,review,preflight:pre})
  impl = result.impl
}
for (let implementAttempt = 0; !impl?.done && implementAttempt <= ADVISOR_MAX; implementAttempt++) {
...
const runVerification=async()=>await llmAgent(
  [
    ...
    `核对实际 git diff 与各切片文件归属；有未实现或越界修改则 red。切片归属=${JSON.stringify(effectivePlan.slices.map(s=>({id:s.id,files:s.files})))}`,
    '',
