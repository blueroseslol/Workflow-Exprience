...
const CHECKPOINT_KEY = workflowCheckpointKey(`${CHANGE_DIR}::${MILESTONE}`, args)
...
extendSliceExecutionSchema(PLAN_SCHEMA)
for (const key of ['milestoneTaskIds', 'completedTaskIds', 'milestoneEvidence']) {
  RECON_SCHEMA.properties.openspec.required.push(key)
  RECON_SCHEMA.properties.openspec.properties[key] = { ...S_STR_ARR, description: key === 'milestoneTaskIds' ? '目标 milestone 的完整 task ID 集合，不能只列当前切片' : key === 'completedTaskIds' ? 'Recon 时该里程碑已完成的 task IDs，亲自核对 tasks.md 与已有完成证据' : '完整里程碑范围及已完成项的 tasks.md 行号/完成证据' }
}
const REVIEW_SCHEMA = {
  type:'object',additionalProperties:false,
...
VERIFY_SCHEMA.required.push('milestoneTaskIds','milestoneEvidence')
VERIFY_SCHEMA.properties.milestoneTaskIds={...S_STR_ARR,description:'重新读取 tasks.md，列出目标 milestone 全部 task IDs，不能仅列当前实现部分'}
VERIFY_SCHEMA.properties.milestoneEvidence={...S_STR_ARR,description:'里程碑任务范围与完成证据的 file:line/命令输出'}
...
const makeReviewInput=(mode,currentPlan,currentChanged)=>stableStringify({
  contract:CHECKPOINT_TEMPLATE_KIND,reviewPolicy: 'milestone-v3',reviewTiming:args.reviewTiming??'milestone',
  task:TASK,
  milestone:MILESTONE,
...
reviewDecision=decideOpenSpecReview(args,recon,plan,effectiveRoute,architectChoices.length>0)
...
const implementationSchedule=buildImplementationSchedule(plan,args)
log(`Implement mode=${implementationSchedule.mode}: ${implementationSchedule.reason}`)
if(implementationSchedule.mode==='blocked')return checkpointEnvelope({status:'blocked',at:'ImplementationSchedule',reason:implementationSchedule.reason})
const implementation=implementationSchedule.mode==='parallel'
  ? await runParallelImplementation(plan,{args,model:implementationModel,schema:IMPLEMENT_SCHEMA,context:`repo=${REPO}; worktree=${WORKTREE}; GitNexus repo=${GNX}; tasks=${TASKS_DOC}; milestone=${MILESTONE}`},implementationSchedule)
  : await implementWithAdvisor(['你是实现层。严格按 OpenSpec execution overlay 实现。',digest(plan),'只改 whitelist；mustNotTouch 禁止触碰。不 commit/push、不提前勾 task，不改变 requirement/design。',K_FAIL_LOUD].join('\n'),{args,model:implementationModel,schema:IMPLEMENT_SCHEMA})
...
const milestoneReview=milestoneReviewState(recon,verify)
const needsAudit=milestoneReview.complete || routeMiss || (args.reviewTiming==='plan' && (effectiveRoute==='CRITICAL'||architectChoices.length>0||advisorCalls>0||repairHistory.length>=2))
let audit=null
if(needsAudit){
  phase('Audit')
  audit=await llmAgent(['你是里程碑 Review（Final Audit）。亲自读取 OpenSpec requirements/design/tasks 和实现/关键 caller，检查整个里程碑的交付；此前切片也须核对，不能仅看本轮 diff 或摘要。',`tasks=${TASKS_DOC}; milestone=${MILESTONE}; scope=${JSON.stringify(milestoneReview)}`,`route=${effectiveRoute}; routeMiss=${routeMiss}`,digest(plan),`verify=${verify?.status??'?'} ${verify?.testPassed??'?'}/${verify?.testTotal??'?'}`, '只读审查行为、契约与完成证据，不重复已通过且未变化的测试。发现缺陷返回 needs-rework 与具体证据；不得写文件/自行返工。'].filter(Boolean).join('\n'),{label:'final-audit',phase:'Audit',effortRole:'Audit',model:MODEL_REVIEW,effort:'high',disallowedTools:['Edit','Write','Agent','Task','Workflow'],schema:AUDIT_SCHEMA})
...
return checkpointEnvelope({
  ...
  milestoneReview:{...milestoneReview,status:milestoneReview.complete?(audit?.verdict==='accept'?'accepted':'needs-rework'):milestoneReview.status},
